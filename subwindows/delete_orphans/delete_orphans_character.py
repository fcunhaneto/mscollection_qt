...
import texts
from db.db_model import Character, Cast
from db.db_settings import Database as DB
from lib.function_lib import hbox_create, pb_create, delete_orphans


class DeleteOrphansCharacter(QMdiSubWindow):
    """
    Class for delete characters who are orphan in database.

    :param main: Reference for main windows.
    """
    def __init__(self, main):
        super(DeleteOrphansCharacter, self).__init__()

        self.session = DB.get_session()
        self.characters = self.session.query(Character)
        self.main = main

        window_title = texts.delete_orphans + ' ' + texts.character_p
    ...
    def create_table(self):
        ...
        """
        sub = self.session.query(Cast.character_id)
        sub = sub.distinct()
        character_result = self.characters.\
            filter(Character.id.notin_(sub)).all()

        for character in character_result:
            self.table.insertRow(self.rows)
            self.table.setItem(self.rows, 0,
                               QTableWidgetItem(character.name))

            ch_del = QCheckBox(str(character.id))
    ...
    def delete(self):
        ...
        """
        delete_orphans(self.session, self.ch_del, Character, texts.character_s)
        self.clear()
        self.create_table()
