...
import texts
from db.db_model import Movie, Series, Category
...
class DeleteOrphansCategory(QMdiSubWindow):
    ...
    def create_table(self):
        ...
        """
        category_movies_1 = self.session.query(Movie.category_1_id).filter(
            Movie.category_1_id.isnot(None))
        sub_movies_1 = category_movies_1.distinct().all()

        category_movies_2 = self.session.query(Movie.category_2_id).filter(
            Movie.category_2_id.isnot(None))
        sub_movies_2 = category_movies_2.distinct().all()

        category_series_1 = self.session.query(Series.category_1_id).filter(
            Series.category_1_id.isnot(None))
        sub_series_1 = category_series_1.distinct().all()

        category_series_2 = self.session.query(Series.category_2_id).filter(
            Series.category_2_id.isnot(None))
        sub_series_2 = category_series_2.distinct().all()

        categories_id = set(
            sub_movies_1 + sub_movies_2 + sub_series_1 + sub_series_2 )

        sub_categories = [c[0] for c in categories_id]

        categories = self.session.query(Category).\
            filter(Category.id.notin_(sub_categories))
