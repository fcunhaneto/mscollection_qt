...
import texts
from db.db_model import Box, Movie
from db.db_settings import Database as DB

from lib.function_lib import hbox_create, pb_create, delete_orphans


class DeleteOrphansBox(QMdiSubWindow):
    def __init__(self, main):
        """
        Class for delete movie box who are orphan in database.

        :param main: Reference for main windows.
        """
        super(DeleteOrphansBox, self).__init__()
    ...
    def create_table(self):
        """
        Create a table for show all orphan box info and with a QCheckBox
        that if is check the actor will be delete.
        """
        sub = self.session.query(Movie.box_id).filter(Movie.box_id.isnot(None))
        sub = sub.distinct()
        box_result = self.session.query(Box).filter(Box.id.notin_(sub)).all()

        for box in box_result:
            self.table.insertRow(self.rows)
            self.table.setItem(self.rows, 0, QTableWidgetItem(box.name))

            ch_del = QCheckBox(str(box.id))
    ...
    def delete(self):
        ...
        """
        delete_orphans(self.session, self.ch_del, Box, texts.box)
        self.clear()
        self.create_table()
